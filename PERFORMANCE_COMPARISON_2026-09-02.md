...
3. 本报告完成后的启动器 v2 已把安全预设、FPS、持续占空目标以及 RAM/VRAM 用途、容量估算和 Raw/Brotli 取舍集中到“性能与安全”页，并在设置页旁加入最高 512²/30 FPS、无需等待 UI 的内嵌预览；这部分不包含在原整体基准中，另有 CPU 微基准覆盖帧拷贝、缩放/颜色转换和共享内存发布，但不包含 wx 绘制。模型与外部输出像素路径不变。

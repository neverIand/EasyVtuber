...
仓库保留了可重复使用的界面与预览回归工具，后续无需再临时生成截图脚本。`benchmarks/capture_launcher_ui.py`可指定标签页、DIP 窗口尺寸、示例角色图和 PNG 输出路径；`benchmarks/benchmark_preview_path.py`会分别测量输出槽内的原始帧拷贝、释放输出槽后的缩放/颜色转换/共享内存发布；`benchmarks/benchmark_preview_refresh.py`会向真实共享内存和 wx 预览定时器发送合成帧，分别报告源、发布端和界面消费帧率。这些工具都不启动 GPU 模型。例如：
...
envs\python_embedded\python.exe benchmarks\benchmark_preview_refresh.py --source-fps 30
envs\python_embedded\python.exe benchmarks\benchmark_preview_refresh.py --source-fps 60 --publish-cap 30
```

`运行安全预设`只组合 FPS 与 GPU 持续占空目标，不会修改模型、Full/Half 精度、输入简化、缓存容量、补帧或超分：

| 预设 | FPS | GPU 持续占空目标 | 用途 |
|---|---:|---:|---|
| 保守 | 24 | 70% | 温度敏感、边直播边游戏时优先使用 |
| 平衡 | 30 | 80% | 新配置默认值 |
| 性能 | 30 | 90% | 散热稳定且需要更多吞吐余量时使用 |
| 自定义 | 手动 | 手动 | 保留其他组合 |

预设下方紧接着显示`输出帧率`和`GPU 持续占空目标`，因此选择`自定义`后可以原地调整。手动修改任一项时，预设会自动切换到`自定义`；如果数值重新匹配某个内置组合，则会自动识别回对应预设。

旧版 `launcher.json` 会自动迁移：原来已经选定的模型、缓存和输入简化值保持不变，再根据现有 FPS/占空目标显示相应的安全预设。配置文件使用临时文件原子替换，损坏或缺失时才回退到新默认值。`恢复默认`按钮只先改变界面并要求确认，点击`保存并启动`后才会写入文件。

#### RAM 与 VRAM 缓存

`RAM 最终帧缓存`是普通帧与超分帧共享的总预算。关闭超分时，预算全部用于 512² BGRA 帧；开启超分后按单帧字节量 1:4 分给 512² 与 1024² 帧。启动器显示的是估算值而非承诺，例如 2 GiB Raw 理论上约容纳 2048 张 512² BGRA 帧，实际容量会受缓存条目和进程开销影响。关闭输入简化时，最终帧缓存不会启用。

`RAM 缓存存储模式`提供两个无损选项：`Raw`命中时无需同步解压，延迟最低但长会话占用更多内存；`Brotli`压缩后通常能容纳更多姿态，但每次命中需要解压。实测性能与内存取舍见[当前版与官方版整体性能比较](PERFORMANCE_COMPARISON_2026-09-02.md)。

`VRAM 模型中间缓存`仅 TensorRT 生效，与 RAM 最终帧缓存用途不同；其活动缓存和复用池共同受所选容量严格限制。

#### GPU 持续负载限制

`GPU 持续占空目标`限制同步模型推理调用的持续占空比。启动器的新配置默认使用平衡预设的`80%`；直接运行命令行时仍默认`90%`。限速器会把模型计算之外已经发生的输出等待计入空闲时间，因此与 FPS 限制配合时不会重复等待。`100%（关闭限制）`可关闭此保护。
...
显卡选择不改变 Full/Half 模型精度，但不同厂商或不同 GPU 的浮点实现可能使少量最终 8-bit 像素发生硬件相关的舍入差异；需要严格复现某次 DirectML 输出时，应固定同一适配器。`GPU 持续占空目标`同样作用于 DirectML。

#### TensorRT 磁盘缓存

内容寻址的 `.trt` 引擎缓存会持久保存并复用，避免每次启动重建模型。启动器在 TensorRT 启动前显示缓存数量、容量、路径和可能重建的风险，也提供只删除受管理缓存文件的安全清理入口。

当前 TensorRT-RTX 1.3 在本机读写附加的 runtime JIT cache 时出现过原生调用无限阻塞，因此这层小缓存默认禁用且不在普通启动器中提供开关。它不影响 `.trt` 引擎、稳定推理速度或像素，只会使每次 TensorRT 启动重新创建 execution context（当前 THA3 FP32 约十几秒）。
